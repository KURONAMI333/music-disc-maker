...
public class DiscSoundInstance extends AbstractTickableSoundInstance {
    ...
    private float computeVolume() {
        return (float) (volumePercent / 100.0 * Config.VOLUME_MULTIPLIER.get());
    }
    ...
    private int effectiveRange() {
        if (rangeBlocks > 0) {
            return Math.min(rangeBlocks, Config.MAX_PLAYBACK_RANGE.get());
        }
        return Config.PLAYBACK_RANGE.get();
    }
     ...
     * mixin ({@link SoundEngineChannelAccess}) 経由で再生中チャンネルへ書き込む。
    ...
    public void requestStop() {
        this.stop();
        // stream 開栓前 (getStream 未呼び出し) に停止すると、MC が AudioStream.close を
