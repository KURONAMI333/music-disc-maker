...
# 指向性トグル (16x16, v=24 の段)。無彩色・枠なし・素置き。
# OFF = 枠の中心にある点 (どの壁からも等距離 = 位置に関係なくフラット)、
# ON = 同じ点が隅に寄る (位置で聴こえ方が変わる)。
SPR_DIR_V = 24
...
DIR_GLYPH = (72, 72, 72, 255)      # 指向性トグルのグリフ (無彩色・単色)
...
# 指向性 OFF = 点が枠の中心 (どの壁からも等距離 = 範囲内ならどこでも同じ)。
_ART_DIR_OFF = """
................
................
................
...
def sprite_directional(on):
    """指向性トグルのグリフ。形だけで state を表す (無彩色・単色)。"""
    return _paint_art(_ART_DIR_ON if on else _ART_DIR_OFF, DIR_GLYPH, DIR_GLYPH)
...
def print_table():
    ...
    print(f"  volume      = {VOL}")
    print(f"  range       = {RANGE}")
    print(f"  directional = {DIRECTIONAL}   sprites "
          f"u={SPR_DIR_ON_U}/{SPR_DIR_OFF_U} v={SPR_DIR_V}")
