...
- **Fixed a disc sometimes staying silent with no reason shown on screen.** Taking it back out and putting it in again usually made it play, but the first attempt could give up without saying why. The disc now plays through instead, and if it truly cannot play, the reason is shown.
...
- **Playback failures now appear on screen instead of in chat.** The Golden Jukebox and Music Disc Maker show a short reason; playback with no screen writes it to the log, with the full detail still available there.
- **Failure reasons are now accurate.** A source that answered with an error no longer looks like a network or DNS problem, and bot check messages no longer imply that only servers can be affected.
- **Albums and repeat no longer pause to load between tracks.** The next track is fetched and buffered before the current one ends, so the wait to look it up is gone. Silence that is part of the recording itself, such as a few quiet seconds at the end of a song, still plays out as it was recorded.
- **Custom discs now play in Additional Additions' Pocket Jukebox.**
- Fixed a startup library unpack failure preventing every later playback attempt until the game was restarted.
- Made the Music Disc Maker failure message easier to read and kept the failed URL after its screen is closed.
- Fixed Create contraptions and airships leaving music stopped forever after one failed attempt.
