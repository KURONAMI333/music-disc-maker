...
- **Golden Jukebox** — a new craftable block (surround a jukebox with gold ingots) with a settings GUI: adjust the audible range (16-256 blocks), volume (0-200%), toggle repeat, and pause/resume. Holds any disc; the range and volume apply per block so you can make one jukebox fill a large area and another stay quiet
