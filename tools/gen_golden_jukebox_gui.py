...
# transport (メインコントロール列)。囲み枠は持たない (KURONAMI333 高評価の前デザイン = 素の配置)。
...
GOLD = (206, 168, 68, 255)         # アクセント (KURONAMI333 高評価)
...
# 再生/一時停止は KURONAMI333 高評価の前デザイン (金の実心ディスク + 暗い記号くり抜き) を
...
def main():
    # 26.1.2 は単一 src/ レイアウト (common/ 分割なし)。出力先は repo ルート起点で解決する。
