 ...
 * {@link AlbumProvider} を {@link AdditionalAdditionsAlbumSupport} へ差し込み、強化版ジュークボックスがアルバムを
 * 受け入れてトラックを送れるようにする。
 *
 * <p>AA のクラス参照 ({@link AlbumProvider}) は {@code isLoaded()} ガードを通過した後の
 * enqueueWork ラムダ内でのみ class-load される ({@code SophisticatedCoreCompat} と同型のパターン)。
