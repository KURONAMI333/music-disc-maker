 ...
 * を直接叩くだけで、SB のような近隣プレイヤーへの broadcast packet が無い) ので、この compat も同じ
 * 「クリックした本人にしか聞こえない」聞こえ方に揃える (TB 自体の設計がそうなっているだけで、機能を
 * 削っているわけではない)。
 *
 ...
 *
 * <p>26.2 の TB は {@code JukeboxUpgrade.diskHandler} が NeoForge の新 transfer API
 * ({@code ItemStacksResourceHandler}) になっており {@code getStackInSlot(int)} が存在しない
 * (1.21.1 版 TB からの破壊的変更)。TB 自身が内部で使っている公開 util
 * {@link StacksHandlerUtils#getStackInSlot} で読み出す (TB 実装が変わってもこの経路は追従する)。
 */
public final class TravelersBackpackCompatClient {
    ...
    public static void onPlayClicked(int entityId) {
        ...
        POOL.submit(() -> {
            ...
            Minecraft.getInstance().execute(() -> {
                if (resolved == null) {
                    // 無音で終わらせない。理由の分類つきで画面とログの両方に残す (本体の再生経路と同じ出方)。
     ...
     *
     * <p>{@code diskHandler} は NeoForge の新 transfer API ({@code ItemStacksResourceHandler}) で
     * {@code getStackInSlot(int)} を持たない (旧 {@code ItemStackHandler} との相違点)。代わりに
     * {@code copyToList()} でスナップショットを取り、先頭スロットを読む。
     */
    @Nullable
    private static ItemStack currentDisc() {
        ...
        //? if >=26.2 {
        return upgrade.map(u -> StacksHandlerUtils.getStackInSlot(u.diskHandler, 0)).orElse(null);
        //?} elif >=26.1 {
        /*if (upgrade.isEmpty()) {
            return null;
        }
        final List<ItemStack> stacks = upgrade.get().diskHandler.copyToList();
