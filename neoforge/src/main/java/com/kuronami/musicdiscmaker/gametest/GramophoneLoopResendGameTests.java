 ...
 * 行う操作 —「repeat 復帰の tick で {@link ExternalPlaybackMirror#mirror} を stop→play の順で
 * 2回呼ぶ」— を {@link ExternalPlaybackMirror} の公開 API だけを使って直接再現する。これにより
...
public class GramophoneLoopResendGameTests {
     ...
     * 毎 tick 呼んでも音声が暴れる前提) を確認する。続けて、{@code FurnitureGramophoneMixin} が
    ...
    *///?}
    public static void repeatRestartMarkerForcesStopThenPlayResend(GameTestHelper helper) {
        ...
        withCapturedNetwork(net -> {
            ...
            ExternalPlaybackMirror.mirror(level, pos, null);
            ExternalPlaybackMirror.mirror(level, pos, disc);
