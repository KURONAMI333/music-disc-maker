 ...
 * {@link AdditionalAdditionsAlbumSupport} の差し込み口へ「バンドルをアルバムと見なす」fake を入れて、common 側の
...
*///?}
public class AdditionalAdditionsAlbumGameTests {
    ...
    private static final class FakeAlbums implements AdditionalAdditionsAlbumSupport.Provider {
    ...
    private static void installFakeAlbums() {
        AdditionalAdditionsAlbumSupport.install(new FakeAlbums());
    }

    //? if <1.21.2 {
    /*@AfterBatch(batch = BATCH)
    public static void restoreAlbums(ServerLevel level) {
        AdditionalAdditionsAlbumSupport.install(null);
    }
    ...
    *///?} else {
    private static ItemStack customDisc(GameTestHelper helper, String url, String title, long durationMs) {
        final ItemStack disc = new ItemStack(ModItems.CUSTOM_MUSIC_DISC.get());
        disc.set(ModDataComponents.CUSTOM_TRACK.get(),
                new CustomTrackData(url, title, "tester", durationMs, "", false));
    ...
    private static ItemStack album(ItemStack... discs) {
        ...
        //? if >=26.1 {
        // 26.1+ の BundleContents は ItemStackTemplate を取る (ItemStack から fromNonEmptyStack で変換)。
        stack.set(DataComponents.BUNDLE_CONTENTS,
                new BundleContents(java.util.Arrays.stream(discs)
    ...
    *///?}
    public static void capturedVanillaAlbumAdvancesAndRepeats(GameTestHelper helper) {
        installFakeAlbums();
        final var source = placeJukebox(helper, new BlockPos(1, 1, 1));
        if (source == null) return;
        source.setItem(0, album(new ItemStack(Items.MUSIC_DISC_CAT), new ItemStack(Items.MUSIC_DISC_13)));
        final var level = helper.getLevel();
        final var data = source.saveWithFullMetadata(level.registryAccess());
        data.putBoolean("repeat", false);
        final var first = com.kuronami.musicdiscmaker.component.CapturedGoldenPlayback
                .read(level, BlockPos.ZERO, data).orElseThrow();
        final var second = com.kuronami.musicdiscmaker.component.CapturedGoldenPlayback
                .advance(level, BlockPos.ZERO, data, first.vanilla().durationMs()).orElseThrow();
        helper.assertTrue(second.cursor().discIndex() == 1 && second.offsetMs() == 0L
                        && "minecraft:music_disc.13".equals(second.vanilla().soundEventId())
                        && second.cursor().generation() > first.cursor().generation(),
                "Captured compatibility album did not advance to the second vanilla disc");
        final var looping = data.copy();
        looping.putBoolean("repeat", true);
        final var repeated = com.kuronami.musicdiscmaker.component.CapturedGoldenPlayback
                .advance(level, BlockPos.ZERO, looping, second.vanilla().durationMs()).orElseThrow();
        helper.assertTrue(repeated.cursor().discIndex() == 0 && repeated.offsetMs() == 0L
                        && repeated.vanilla().equals(first.vanilla())
                        && repeated.sourceId().equals(first.sourceId()),
                "Captured compatibility album did not repeat from the first vanilla disc");
        helper.assertTrue(com.kuronami.musicdiscmaker.component.CapturedGoldenPlayback
                        .advance(level, BlockPos.ZERO, data, second.vanilla().durationMs()).isEmpty(),
                "Captured compatibility album did not stop at the end without repeat");
        helper.succeed();
    }
    ...
    *///?}
    public static void albumIsAccepted(GameTestHelper helper) {
        ...
        *///?} else {
        final ItemStack albumStack = album(customDisc(helper, URL_A, "A", 120_000L), customDisc(helper, URL_B, "B", 120_000L));
        //?}
    ...
    *///?}
    public static void effectiveDiscIsCurrentAlbumTrack(GameTestHelper helper) {
        ...
        *///?} else {
        withAlbum.setItem(GoldenJukeboxBlockEntity.SLOT_DISC,
                album(customDisc(helper, URL_A, "A", 120_000L), customDisc(helper, URL_B, "B", 120_000L)));
        //?}
        ...
        *///?} else {
        withDisc.setItem(GoldenJukeboxBlockEntity.SLOT_DISC, customDisc(helper, URL_B, "B", 120_000L));
        //?}
    ...
    *///?}
    public static void albumAdvancesToNextTrack(GameTestHelper helper) {
        ...
        *///?} else {
        jukebox.setItem(GoldenJukeboxBlockEntity.SLOT_DISC,
                album(customDisc(helper, URL_A, "A", 1L), customDisc(helper, URL_B, "B", 120_000L)));
        //?}
    ...
    *///?}
    public static void albumTrackAndOffsetSurviveReload(GameTestHelper helper) {
        ...
        helper.startSequence()
                ...
                .thenExecute(() -> {
                    ...
                    jukebox.seekTo(30_000L); // Keep a deterministic nonzero audio offset in accelerated GameTests.
                    ...
                    final long restoredElapsed = restored.currentElapsedMs();
                    helper.assertTrue(restoredElapsed <= elapsedBefore && elapsedBefore - restoredElapsed <= 50L,
                            "復元後の再生位置が保存gameTimeの50ms粒度を超えてずれた");
                })
                .thenSucceed();
        *///?} else {
        jukebox.setItem(GoldenJukeboxBlockEntity.SLOT_DISC,
     ...
     * <b>ミラーの入口</b>を直接叩いて分岐を固定する。{@link AdditionalAdditionsAlbumSupport} に fake を差してあるので
    ...
    *///?}
    public static void mirrorLeavesPlainDiscJukeboxesAlone(GameTestHelper helper) {
        ...
        *///?} else {
        final ItemStack plainDisc = customDisc(helper, URL_A, "A", 120_000L);
        //?}
        ...
        try {
            ...
            *///?} else {
            ...
            ExternalPlaybackMirror.mirrorAlbumJukebox(level, pos, plainDisc, null);
            ...
            *///?} else {
            final ItemStack albumStack = album(customDisc(helper, URL_B, "B", 120_000L));
            //?}
            net.clear();
            ExternalPlaybackMirror.mirrorAlbumJukebox(level, pos, albumStack, null);
            //? if <1.21.2 {
            ...
            *///?} else {
            ...
            net.clear();
            //? if <1.21.2 {
            /*ExternalPlaybackMirror.mirrorAlbumJukebox(level, pos, albumStack, customDisc(URL_B, "B", 120_000L));
            *///?} else {
            ExternalPlaybackMirror.mirrorAlbumJukebox(level, pos, albumStack, customDisc(helper, URL_B, "B", 120_000L));
            //?}
