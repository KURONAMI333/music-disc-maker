...
public class PlaybackPrefetchGameTests {
    ...
    *///?}
    public static void aWarmSourceIsHandedToThePlaybackThatAsksForIt(GameTestHelper helper) {
        ...
        final PlaybackPrefetch.Ticket<String> ticket = prefetch.begin(KEY, URL_A, 1L);
    ...
    *///?}
    public static void changingTheNextTrackClosesTheOldPrefetch(GameTestHelper helper) {
        ...
        prefetch.deliver(prefetch.begin(KEY, URL_A, 1L), first);

        final PlaybackPrefetch.Ticket<String> second = prefetch.begin(KEY, URL_B, 1L);
    ...
    *///?}
    public static void repeatedRequestsForTheSameTrackDoNotStack(GameTestHelper helper) {
        ...
        for (int i = 0; i < 20; i++) {
            helper.assertTrue(prefetch.begin(KEY, URL_A, 1L) == null, "同じ曲の先読みを重ねている");
        }
        final FakeSource warm = new FakeSource();
    ...
    *///?}
    public static void anExpiredPrefetchIsClosedAndNeverHandedOut(GameTestHelper helper) {
        ...
        prefetch.deliver(prefetch.begin(KEY, URL_A, 1L), stale);
        ...
        prefetch.deliver(prefetch.begin(KEY, URL_B, 1L), orphan);
        clock.addAndGet(PlaybackPrefetch.EXPIRY_MS);
        prefetch.begin(KEY, URL_A, 1L); // 次の先読みの入口で掃除が走る
    ...
    *///?}
    public static void aRequestWithAnOffsetDoesNotGetTheHeadOfTheTrack(GameTestHelper helper) {
        ...
        final FakeSource warm = new FakeSource();
        prefetch.deliver(prefetch.begin(KEY, URL_A, 1L), warm);
    ...
    *///?}
    public static void everyCancellationPathClosesExactlyOnce(GameTestHelper helper) {
        ...
        final PlaybackPrefetch.Ticket<String> cancelled = prefetch.begin(KEY, URL_A, 1L);
        prefetch.drop(KEY);
        final FakeSource late = new FakeSource();
    ...
    *///?}
    public static void newPrefetchesAreNotStartedOnceTheLimitIsReached(GameTestHelper helper) {
        ...
        for (int i = 0; i < PlaybackPrefetch.MAX_CONCURRENT_PREFETCH; i++) {
            final PlaybackPrefetch.Ticket<String> ticket = prefetch.begin("jukebox-" + i, URL_A, 1L);
            helper.assertTrue(ticket != null, "上限に達する前の先読みが始まっていない: " + i);
        }

        helper.assertTrue(prefetch.begin("jukebox-overflow", URL_A, 1L) == null,
        ...
        final PlaybackPrefetch.Ticket<String> swapped = prefetch.begin("jukebox-0", URL_B, 1L);
    ...
    *///?}
    public static void singleTrackRepeatWarmsTheUrlItIsAlreadyPlaying(GameTestHelper helper) {
        ...
        prefetch.deliver(prefetch.begin(KEY, URL_A, 1L), firstLoop);
        helper.assertTrue(prefetch.claim(KEY, URL_A, 0L) == firstLoop, "折り返し用に温めた同じ URL が掴めていない");

        // 2 周目の終わり際: 掴んだ直後に、同じ URL の先読みをもう一度始められること。
        final PlaybackPrefetch.Ticket<String> nextLoop = prefetch.begin(KEY, URL_A, 1L);
    ...
    *///?}
    public static void aRestartedTrackKeepsThePrefetchMadeForItsCurrentUrl(GameTestHelper helper) {
        ...
        helper.assertTrue(prefetch.deliver(prefetch.begin(KEY, URL_A, 1L), warm), "先読みを準備できる");
