    ...
    *///?} else {
    private static ItemStack customDisc(GameTestHelper helper) {
        ...
        helper.getLevel().registryAccess().lookupOrThrow(Registries.JUKEBOX_SONG)
                .get(SilentSongs.pick(DURATION_MS))
                //? if >=26.1 {
    ...
    public static void playPayloadCarriesFlatMode(GameTestHelper helper) {
        ...
        *///?} else {
        ...
        withCapturedNetwork(net -> {
            ...
            *///?} else {
            jukebox.setItem(GoldenJukeboxBlockEntity.SLOT_DISC, customDisc(helper));
            //?}
    ...
    *///?}
    public static void playPayloadCodecKeepsDirectional(GameTestHelper helper) {
        for (final boolean directional : new boolean[] { true, false }) {
            ...
            mdmCodec(PlayDiscPayload::read).encode(buf, sent);
            final PlayDiscPayload received = mdmCodec(PlayDiscPayload::read).decode(buf);
     ...
     *
     * <p>26.x 帯には Create compat が未展開 ({@code ContraptionPlayDiscPayload} の 26.x 腕が空) のため
     * このテストは 1.21.1 専用で残す。
     */
    //? if <1.21.2 {
    /*@PrefixGameTestTemplate(false)
    ...
    public static void contraptionPayloadCodecKeepsDirectional(GameTestHelper helper) {
        for (final boolean directional : new boolean[] { true, false }) {
            final ContraptionPlayDiscPayload sent = new ContraptionPlayDiscPayload(
                    ...
                    new com.kuronami.musicdiscmaker.network.PlaybackSourceStamp(
            ...
            helper.assertTrue(received.identity().equals(sent.identity()),
                    "Create payloadの音源IDまたは世代が往復で失われた");
        ...
        final var stop = ContraptionPlayDiscPayload.stop(77, new BlockPos(1, 2, 3),
                new com.kuronami.musicdiscmaker.network.PlaybackSourceStamp(
                        java.util.UUID.fromString("d89e535b-1266-4a97-a098-fcd66390a0b0"), 917L));
        final var vanilla = new ContraptionPlayDiscPayload(77, stop.localPos(), CustomTrackData.EMPTY,
                17500L, 128, 75, true, stop.identity(),
                new com.kuronami.musicdiscmaker.component.VanillaTrackData("minecraft:music_disc.cat", 185000L),
                new com.kuronami.musicdiscmaker.network.MovingSpeakerState(new net.minecraft.world.phys.Vec3(4000, 64, 4000),
                        java.util.List.of(new com.kuronami.musicdiscmaker.network.SpeakerEntry(new BlockPos(0, 64, 0), 80, false, 35))));
        helper.assertTrue(vanilla.receivers().reaches(new net.minecraft.world.phys.Vec3(1, 64, 1), 128, 75, false),
                "Listener beside a fixed speaker was excluded because the moving source is far away");
        helper.assertTrue(!vanilla.receivers().reaches(new net.minecraft.world.phys.Vec3(2000, 64, 2000), 128, 75, true),
                "Listener outside both source and speaker envelopes was retained");
        final var muted = new com.kuronami.musicdiscmaker.network.MovingSpeakerState(vanilla.receivers().sourcePosition(),
                java.util.List.of(new com.kuronami.musicdiscmaker.network.SpeakerEntry(new BlockPos(0, 64, 0), 80, true, 35)));
        helper.assertTrue(!muted.reaches(new net.minecraft.world.phys.Vec3(1, 64, 1), 128, 75, false)
                        && muted.reaches(new net.minecraft.world.phys.Vec3(1, 64, 1), 128, 75, true),
                "Mute must reject a new listener while preserving an existing voice for gain updates");
        final ByteBuf vanillaBuf = Unpooled.buffer();
        try {
            ContraptionPlayDiscPayload.STREAM_CODEC.encode(vanillaBuf, vanilla);
            final var decodedVanilla = ContraptionPlayDiscPayload.STREAM_CODEC.decode(vanillaBuf);
            helper.assertTrue(!decodedVanilla.isStop() && decodedVanilla.equals(vanilla)
                            && vanillaBuf.readableBytes() == 0,
                    "Create vanilla payload was treated as stop or lost track/position/identity data");
        } finally {
            vanillaBuf.release();
        }
        final ByteBuf stopBuf = Unpooled.buffer();
        try {
            ContraptionPlayDiscPayload.STREAM_CODEC.encode(stopBuf, stop);
            final var decoded = ContraptionPlayDiscPayload.STREAM_CODEC.decode(stopBuf);
            helper.assertTrue(decoded.isStop() && decoded.equals(stop) && stopBuf.readableBytes() == 0,
                    "Create stop lost actor position or source identity during codec round trip");
        } finally {
            stopBuf.release();
        }
        helper.succeed();
    ...
    public static void subLevelPayloadCodecKeepsDirectional(GameTestHelper helper) {
        for (final boolean directional : new boolean[] { true, false }) {
            final SubLevelPlayDiscPayload sent = new SubLevelPlayDiscPayload(
                    ...
                    new com.kuronami.musicdiscmaker.network.PlaybackSourceStamp(
                            java.util.UUID.fromString("ab20d9c8-1ab7-4786-aa29-e8a2d34d12bf"), 923L));
            final ByteBuf buf = Unpooled.buffer();
            ...
            helper.assertTrue(received.identity().equals(sent.identity()) && buf.readableBytes() == 0,
                    "Sable の音源 ID と再生世代が wire で変わっている");
            final var stop = new com.kuronami.musicdiscmaker.compat.aeronautics.SubLevelStopDiscPayload(
                    sent.plotPos(), sent.identity());
            com.kuronami.musicdiscmaker.compat.aeronautics.SubLevelStopDiscPayload.STREAM_CODEC.encode(buf, stop);
            final var stopReceived = com.kuronami.musicdiscmaker.compat.aeronautics.SubLevelStopDiscPayload.STREAM_CODEC.decode(buf);
            helper.assertTrue(stop.equals(stopReceived) && buf.readableBytes() == 0,
                    "Sable の停止対象が wire で変わっている");
            buf.release();
        }
        helper.succeed();
    }
    ...
    *///?}
    public static void nbtRoundTripsDirectionalAndDefaultsToTrue(GameTestHelper helper) {
        ...
        *///?} else {
        ...
        jukebox.setDirectional(false);
        final CompoundTag flat = jukebox.saveWithoutMetadata(registries);
        helper.assertTrue(flat.contains("directional") && !flat.getBooleanOr("directional", false),
                "フラット設定が NBT に保存されていない");
        jukebox.setDirectional(true);
        jukebox.loadWithComponents(TagValueInput.create(ProblemReporter.DISCARDING, registries, flat));
        //?}
        ...
        *///?} else {
        helper.assertTrue(positional.getBooleanOr("directional", false), "positional 設定が NBT に保存されていない");
        jukebox.setDirectional(false);
        jukebox.loadWithComponents(TagValueInput.create(ProblemReporter.DISCARDING, registries, positional));
        //?}
        ...
        *///?} else {
        jukebox.loadWithComponents(TagValueInput.create(ProblemReporter.DISCARDING, registries, legacy));
        //?}
