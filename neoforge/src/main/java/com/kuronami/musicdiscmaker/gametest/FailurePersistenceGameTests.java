...
public class FailurePersistenceGameTests {
    ...
    private static MusicDiscMakerBlockEntity place(GameTestHelper helper, BlockPos rel) {
        ...
        *///?} else {
        final MusicDiscMakerBlockEntity be = helper.getBlockEntity(rel, MusicDiscMakerBlockEntity.class);
        //?}
        if (be == null) {
            //? if <1.21.2 {
            /*helper.fail("MusicDiscMakerBlockEntity が生成されていない", rel);
            *///?} else {
            helper.fail(Component.literal("MusicDiscMakerBlockEntity が生成されていない"), rel);
            //?}
        }
        return be;
    }

    // save は 26.x でも CompoundTag を返す final オーバーロードが残っているためそのまま使える。
    // load のみ CompoundTag 版が無いので TagValueInput.create 経由にする。
    // getBoolean / getString は 26.x で Optional を返すようになったので getBooleanOr / getStringOr を使う。
    ...
    *///?}
    public static void resolveFailedSurvivesSaveAndLoad(GameTestHelper helper) {
        ...
        *///?} else {
        helper.assertTrue(saved.getBooleanOr("resolveFailed", false),
                "resolveFailed が saveAdditional (= saveWithoutMetadata) に書かれていない。"
                        + "getUpdateTag 経由の同期にしか乗らないと、chunk 再ロードで失敗の事実が消える");
        //?}
        //? if <1.21.2 {
        ...
        *///?} else {
        helper.assertTrue(FailureReason.PRIVATE_OR_REMOVED.name().equals(saved.getStringOr("failureReason", "")),
                "failureReason が saveAdditional に書かれていない (実際: " + saved.getStringOr("failureReason", "") + ")");
        //?}
    ...
    *///?}
    public static void legacySaveWithoutFailureTagsDefaultsToNoFailure(GameTestHelper helper) {
        ...
        *///?} else {
        reloaded.loadWithComponents(TagValueInput.create(ProblemReporter.DISCARDING, registries, legacy));
        //?}
