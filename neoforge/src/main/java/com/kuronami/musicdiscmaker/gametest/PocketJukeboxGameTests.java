 ...
 *       復元する読み取り (vanilla の {@code CONTAINER} component と {@link AdditionalAdditionsAlbumSupport} だけを使う)</li>
...
public class PocketJukeboxGameTests {
    ...
    *///?}
    ...
    /** アルバムを飲み込んだ場合は {@link AdditionalAdditionsAlbumSupport} 経由で中身がトラック列になる。 */
    //? if <1.21.2 {
    /*@PrefixGameTestTemplate(false)
    @GameTest(template = TEMPLATE, timeoutTicks = 200, batch = "pocketJukebox")
    *///?}
    public static void albumContentsBecomeTheTrackList(GameTestHelper helper) {
        ...
        final AdditionalAdditionsAlbumSupport.Provider previous = AdditionalAdditionsAlbumSupport.install(new FakeAlbums(album, tracks));
        try {
            ...
            AdditionalAdditionsAlbumSupport.install(previous);
        }
        helper.succeed();
    }
    ...
    *///?}
    ...
    /** 1 つの stack だけをアルバムとして答える {@link AdditionalAdditionsAlbumSupport.Provider}。 */
    private record FakeAlbums(ItemStack album, List<ItemStack> tracks) implements AdditionalAdditionsAlbumSupport.Provider {
