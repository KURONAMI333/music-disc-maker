// 1.21.1 ノード。ModDevGradle 2.0.142 (26.2 ノードと同版に揃える) / NeoForge 21.1.227 / Java 21。
plugins {
    id("java-library")
...
val mcVersionRange = "[1.21.1, 1.22)"
val neoforgeVersion = "21.1.227"
val neoforgeLoaderRange = "[4,)"
val javaVersion = 21
val compatLibsDir = rootProject.file("neoforge/compat-libs-1.21.1")
...
neoForge {
    runs {
        ...
        create("createGameTestServer") {
            gameDirectory.set(layout.projectDirectory.dir("run-gametest-create"))
            type.set("gameTestServer")
            systemProperty("musicdiscmaker.dev", lavaDevPath)
            systemProperty("musicdiscmaker.gametest.create", "true")
            systemProperty("neoforge.enabledGameTestNamespaces", "mdm_create_test")
        }
        create("additionalAdditionsGameTestServer") {
            gameDirectory.set(layout.projectDirectory.dir("run-gametest-additionaladditions"))
            type.set("gameTestServer")
            systemProperty("musicdiscmaker.dev", lavaDevPath)
            systemProperty("neoforge.enabledGameTestNamespaces", "mdm_aa_integration")
        }
...
tasks.matching {
    it.name in listOf("runClient", "runServer", "runGameTestServer", "runCreateGameTestServer", "runAdditionalAdditionsGameTestServer", "runClientProd")
}.configureEach {
    dependsOn(project(":lavaplayer").tasks.named("assemble"))
}

val prepareCreateGameTestMods by tasks.registering(Copy::class) {
    val createJar = compatLibsDir.resolve("create-1.21.1-6.0.10.jar")
    doFirst { check(createJar.isFile) { "Create integration tests require $createJar" } }
    from(createJar)
    into(layout.projectDirectory.dir("run-gametest-create/mods"))
}
tasks.named<ProcessResources>("processResources") {
    // Same empty fixture; a separate namespace selects only real-Create integration tests.
    from(rootProject.file("neoforge/src/main/resources/data/music_disc_maker/structure/empty8x3x8.nbt")) {
        into("data/mdm_create_test/structure")
    }
    from(rootProject.file("neoforge/src/main/resources/data/music_disc_maker/structure/empty8x3x8.nbt")) {
        into("data/mdm_aa_integration/structure")
    }
}
tasks.matching { it.name == "runCreateGameTestServer" }.configureEach {
    dependsOn(prepareCreateGameTestMods)
}

val prepareAdditionalAdditionsGameTestMods by tasks.registering(Copy::class) {
    val additionalAdditionsJar = compatLibsDir.resolve("additionaladditions-neoforge-10.0.5.jar")
    doFirst {
        check(additionalAdditionsJar.isFile) {
            "Additional Additions integration tests require $additionalAdditionsJar"
        }
    }
    from(additionalAdditionsJar)
    into(layout.projectDirectory.dir("run-gametest-additionaladditions/mods"))
}

tasks.matching { it.name == "runAdditionalAdditionsGameTestServer" }.configureEach {
    dependsOn(prepareAdditionalAdditionsGameTestMods)
}
