 ...
 * の分岐そのまま)。前者は vanilla component、後者は既存の {@link AdditionalAdditionsAlbumSupport} アダプタで読めるので、
 * AA 固有の型 ({@code AlbumContents} / {@code PocketJukeboxPlayer}) に触れずに同じ列を再現できる。
 *
 * <p>{@link AdditionalAdditionsAlbumSupport} の実装は loader 側の compat が AA ロード時にだけ差し込む。未導入環境では
...
public final class PocketJukeboxTracks {
    ...
    public static ItemStack trackAt(@Nullable ItemStack pocket, int index) {
        ...
        if (AdditionalAdditionsAlbumSupport.isAlbum(stored)) {
            return AdditionalAdditionsAlbumSupport.trackAt(stored, index);
        }
