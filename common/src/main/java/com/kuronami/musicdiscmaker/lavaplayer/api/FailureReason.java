...
public enum FailureReason {
    ...
    PREVIEW_ONLY,
    /**
     * YouTube 側の事情で解決できなかったため代替ソースを探したところ、<b>同じ録音とみなせる
     * 候補が1つも無かった</b>。検索が実際に走って空振りした時だけこれを出す
     * (曲名が取れず探せなかった時は元の理由のまま。{@code MusicLoaderImpl#substitute} の
     * 最終出口だけで投げ、時間切れや検索不発では投げない)。
     *
     * <p>空振りの主因はリミックスやカバーを意図的に跳ねていることなので
     * (MDM_DECISIONS D19・D45)、<b>別のリンクなら通る可能性が残っている</b>。
     * 文言は「壊れた」「再生不可」でなく「見つからなかった」側に寄せる (D46)。
     * 出し先は {@link #PREVIEW_ONLY} と同じく制作機の GUI ラベルとログ (D22)。
     */
    ...
    public String guiKey() {
        return switch (this) {
            ...
            case NO_ALTERNATIVE_FOUND -> "gui.music_disc_maker.failed.noalt";
        };
    }
