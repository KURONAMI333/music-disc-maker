...
public final class DependencyManager {
    ...
    private static void doLoad() throws IOException {
        ...
        if (devPath != null && !devPath.isBlank()) {
            ...
            install("the dev paths (" + devPath + ")", found);
            return;
        }

        // production で返るのは jar の中を指す Path で、walk も読み出しも jar の
        // FileSystem が開いている間しかできない。展開が終わるまでを丸ごと囲う。
        try (LocatedFolder located = locateInJar("dependencies")) {
            final Path root = located.path();
            install("the mod jar (" + root + ")", walkPacked(root));
        }
    }

    /** 見つけた {@code *.jar.packed} を展開し、揃ったことを確かめてから classloader へ登録する。 */
    private static void install(String searchedIn, Set<Path> packed) throws IOException {
    ...
    private static LocatedFolder locateInJar(String folder) {
        try {
    ...
    /**
     * loader 別に mod jar 内のフォルダ位置を特定する (各ローダー実装は ServiceLoader 登録)。
     *
     * <p>返すのは裸の {@link Path} ではなく {@link LocatedFolder}。production で返る Path は
     * jar の中を指し、その jar の {@link java.nio.file.FileSystem} が開いている間しか使えない。
     * 「誰がいつ閉じるか」を呼び出し側の慣習に委ねると必ず落ちるので、型で運ばせる。
     */
    public interface PathLocator {
        LocatedFolder locate(String modid, String folder) throws IOException, IllegalStateException;
    }
}
