...
public class MusicDiscMakerMenu extends AbstractContainerMenu {
    ...
    //? if >=1.21.2 {
    // クライアント側コンストラクタ (PayloadRegistrar / IMenuTypeExtension 経由)
    public MusicDiscMakerMenu(int containerId, Inventory playerInventory, RegistryFriendlyByteBuf buf) {
    ...
    *///?}
    ...
    public MusicDiscMakerMenu(int containerId, Inventory playerInventory, MusicDiscMakerBlockEntity blockEntity) {
        ...
        blockEntity.returnLegacyMaterials(playerInventory.player);

        // 入力 (空ディスク)
        addSlot(new InputSlot(blockEntity, MusicDiscMakerBlockEntity.SLOT_INPUT,
                MusicDiscMakerLayout.INPUT_X, MusicDiscMakerLayout.INPUT_Y));
        // 出力 (custom disc) — 取り出し専用
        addSlot(new OutputSlot(blockEntity, MusicDiscMakerBlockEntity.SLOT_OUTPUT,
                MusicDiscMakerLayout.OUTPUT_X, MusicDiscMakerLayout.OUTPUT_Y));
        // プレイヤーインベントリ 3 行 (200幅パネルで中央寄せ: x=19)
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                addSlot(new Slot(playerInventory, col + row * 9 + 9,
                        MusicDiscMakerLayout.INVENTORY_X + col * 18,
                        MusicDiscMakerLayout.INVENTORY_Y + row * 18));
            }
        }
        // ホットバー
        for (int col = 0; col < 9; col++) {
            addSlot(new Slot(playerInventory, col,
                    MusicDiscMakerLayout.INVENTORY_X + col * 18, MusicDiscMakerLayout.HOTBAR_Y));
        }
    }

    //? if >=1.21.2 {
    private static MusicDiscMakerBlockEntity resolve(Inventory playerInventory, RegistryFriendlyByteBuf buf) {
    ...
    *///?}
    ...
    public @NotNull ItemStack quickMoveStack(Player player, int index) {
        ...
        } else if (blockEntity.canPlaceItem(MusicDiscMakerBlockEntity.SLOT_INPUT, stack)) {
            // プレイヤー → 入力スロット (空ディスク)
            if (!moveItemStackTo(stack, MusicDiscMakerBlockEntity.SLOT_INPUT,
                    MusicDiscMakerBlockEntity.SLOT_INPUT + 1, false)) {
                return ItemStack.EMPTY;
            }
        } else {
            return ItemStack.EMPTY;
    ...
    /** 入力条件はBlockEntityと共通にし、手動投入も空ディスクだけに限定する。 */
    private static class InputSlot extends Slot {
        private final MusicDiscMakerBlockEntity be;

        InputSlot(MusicDiscMakerBlockEntity be, int slot, int x, int y) {
            super(be, slot, x, y);
            this.be = be;
        }
        ...
        @Override
        public boolean mayPlace(@NotNull ItemStack stack) {
            return be.canPlaceItem(MusicDiscMakerBlockEntity.SLOT_INPUT, stack);
        }
    }
}
