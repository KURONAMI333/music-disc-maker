...
public final class PlaybackPrefetch<K> {
     ...
     *
     * <p>現在の曲が終わっていたら新しい枠を作らず、有効な既存の枠も触らない。曲送りの同期後に
     * 古いインスタンスが呼んでも、次の再生が {@link #claim} する枠を取り消さないため。
     *
     ...
     * @param remainingMs 現在の曲の残り時間 (ms)。0 以下なら始めない
     * @return 発行された引換券。始めないなら {@code null}
     */
    @Nullable
    public Ticket<K> begin(K key, String url, long remainingMs) {
        sweep();
        if (remainingMs <= 0L) {
            return null;
        }
