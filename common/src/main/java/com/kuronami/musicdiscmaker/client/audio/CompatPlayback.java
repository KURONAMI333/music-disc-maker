...
/**
 * compat 経路 (Sophisticated Core / Traveler's Backpack。将来 Create / Create Aeronautics 等の
 * 移動構造物アダプタが増えれば同様) が {@link DiscSoundInstance} を作る唯一の入口。
 *
 * <h2>なぜ構築子を直接使わせないのか</h2>
 * {@code DiscSoundInstance} を作っただけでは、再生スレッドの中で落ちた失敗はどこにも届かない
 * ({@link CompatFaultWiring} の javadoc)。compat 経路は全部その状態で、実機の症状は
