 ...
 * 再読して追従するが、その経路は {@code LiveConfigAnchor} 限定で、移動構造物のアンカーは入らない。
 ...
 *
 * <p>mod-050 (26.2) には現時点で Sable / Create Aeronautics compat が無いため、このクラスを
 * 使う compat 経路はまだ無い。将来その手の移動構造物 compat を足す時のための共通基盤として
 * 先に用意してある (MC 非依存・headless テスト対象)。
 *
...
public final class LivePlaybackRegistry<K> {
    ...
    public Decision request(K key, String url, int rangeBlocks, int volumePercent, boolean directional) {
        ...
        if (slot != null && !slot.voice().isVoiceStopped()) {
    ...
    public int countPlaying() {
        active.entrySet().removeIf(entry -> entry.getValue().voice().isVoiceStopped());
        return active.size();
    }
    ...
    public void stop(K key) {
        ...
        wanted.remove(key);
        forgetFailure(key);
        final Slot slot = active.remove(key);
        if (slot != null) {
            slot.voice().stopAndRelease();
        }
    }
    ...
    public void stopAll() {
        final var voices = java.util.List.copyOf(active.values());
        active.clear();
        pending.clear();
        wanted.clear();
        failures.clear();
    ...
    public boolean install(K key, String url, PlaybackVoice voice) {
        ...
        if (previous != null && !previous.voice().isVoiceStopped()) {
