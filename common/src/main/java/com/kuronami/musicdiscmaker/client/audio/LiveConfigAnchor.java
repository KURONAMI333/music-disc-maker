 ...
 * {@link StaticAnchor} (固定ジューク)。移動構造物の互換アダプタは、ブロックが AIR 化されるか off-map の
 * 別空間へ移るため再読元が client world に無く、実装せず payload の初期値のまま再生する。
 * {@link EntityAnchor} も per-block 設定を持たない。
 *
