 ...
 * {@link StaticAnchor} (固定ジューク)。Create 捕獲式 ({@code ContraptionAnchor}) は組立時にブロックが
 * AIR 化され再読元が無いため実装せず、payload の初期値のまま再生する。Create Aeronautics の変換式
 * ({@code SableSubLevelAnchor}) はブロックが off-map の sub-level plot に実在し client world の同座標
 * には無いため実装しない。{@link EntityAnchor} も per-block 設定を持たない。
 *
 * <p>能力を独立インターフェースにする理由: {@link DiscAnchor} 本体は「位置」と「存在」だけに責務を
 * 限定しており、音源固有の live 再読を持たせない。また互換 mod の型を参照するアンカーを
 * {@link DiscSoundInstance} が concrete instanceof で見ると、当該 mod 非導入でも class-load が走り
 * verification 時にクラス解決を招く恐れがある。common のこの能力インターフェース越しに分岐すれば
