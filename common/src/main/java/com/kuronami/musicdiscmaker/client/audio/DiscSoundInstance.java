...
public class DiscSoundInstance extends AbstractTickableSoundInstance {
    ...
    private float computeVolume() {
        return (float) (volumePercent / 100.0 * Services.CONFIG.volumeMultiplier());
    }

    /**
     * 実効可聴範囲。強化版 (rangeBlocks&gt;0) は per-block 設定を maxPlaybackRange (既定 256) で頭打ちにし、
     * 通常 jukebox の playbackRange には縛られない。無設定は従来通り playbackRange。
     */
    private int effectiveRange() {
        if (rangeBlocks > 0) {
            return Math.min(rangeBlocks, Services.CONFIG.maxPlaybackRange());
        }
    ...
    // loader 固有の SoundEngine mixin (NeoForge=MixinSoundEngine) が SoundInstance#getStream の
    // 拡張点を横取りしてこれを供給する。common では vanilla の getStream override は存在しない。
