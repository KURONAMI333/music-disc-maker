...
public class DiscSoundInstance extends AbstractTickableSoundInstance {
    ...
    /**
     * 任意の {@link DiscAnchor} に張り付く汎用コンストラクタ。移動構造物 (VS2 船 / Create contraption 等) の
     * 互換アダプタが独自 anchor を渡すのに使う。初期 x/y/z は anchor の現在位置で埋める。
     */
    ...
    public void tick() {
        ...
        // (固定ジューク StaticAnchor / VS2 変換式 VSShipAnchor)。Create 捕獲式は AIR 化で再読元が無く
        // 実装せず自然に弾かれる。vanilla jukebox の BE も GoldenJukeboxBlockEntity ではないので弾かれる。
