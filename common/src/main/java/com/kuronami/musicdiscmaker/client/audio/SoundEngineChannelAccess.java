...
public interface SoundEngineChannelAccess {
    ...
    /**
     * engine の streaming channel pool の限界値 = 同時に鳴らせるストリーミング音源の数。
     * {@code Library} の実 pool が返す {@code getMaxCount()} を読み、他 MOD が pool をさらに
     * 拡張した場合もその実値を返す。未初期化または取得不能なら 0。
     */
    int mdm$streamingPoolLimit();
}
