...
public final class CreateAudioMovementBehaviour implements MovementBehaviour {
    ...
    private static void stopPlayback(MovementContext context, SendState state) {
        ...
        for (final var player : ((net.minecraft.server.level.ServerLevel) context.world).players()) {
            if (state.listeners.contains(player.getUUID())) com.kuronami.musicdiscmaker.platform.Services.NETWORK.sendToPlayer(player, payload);
        }
    ...
    public void tick(MovementContext context) {
        ...
        for (final var player : serverLevel.players()) {
            ...
            if (receivers.reaches(player.position(), range, volume, listening)) {
                com.kuronami.musicdiscmaker.platform.Services.NETWORK.sendToPlayer(player, payload);
                present.add(player.getUUID());
            } else if (listening) {
                com.kuronami.musicdiscmaker.platform.Services.NETWORK.sendToPlayer(player, ContraptionPlayDiscPayload.stop(entityId, context.localPos, identity));
            }
        }
        state.listeners.clear();
        state.listeners.addAll(present);
        // 診断 (debug 既定 off): KURONAMI333 実機で「追従しない」時、この行が出て CreateAudioClient.play の
